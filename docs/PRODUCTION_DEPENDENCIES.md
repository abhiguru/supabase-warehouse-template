...
|---|---|---|---|
| 1 | Container vulnerabilities | All-profile scan with fail-closed coverage validation; locally built Edge Runtime and Realtime apply Debian updates to digest-pinned upstream applications. CUPS was already patched. Scan reports must exist, match the image, and contain valid results. | The final 19-image scan passes 15 images. Grafana, Studio and postgres-meta have findings; PostgREST has no package results and fails evidence validation. See [CONTAINER_SECURITY.md](CONTAINER_SECURITY.md). Remaining upstream application/library findings require reviewed upgrades or source rebuilds and compatibility tests. This is **not solely an external-service blocker**. The database major version is preserved. |
