...
compose --profile pooler up -d --wait --wait-timeout 120 supavisor
...
for port in 5432 6543; do
  result=$(compose exec -T db sh -c 'PGPASSWORD="$POSTGRES_PASSWORD" psql -X -h supavisor -p "$1" -U "$2" -d postgres -v ON_ERROR_STOP=1 -Atc "SELECT 1"' sh "$port" "postgres.$tenant")
  [[ "$result" == 1 ]] || { echo "Pooler query failed on port $port" >&2; exit 1; }
  if compose exec -T db sh -c 'PGPASSWORD=warehouse-invalid-probe psql -X -h supavisor -p "$1" -U "$2" -d postgres -v ON_ERROR_STOP=1 -Atc "SELECT 1"' sh "$port" "postgres.$tenant" >/dev/null 2>&1; then
