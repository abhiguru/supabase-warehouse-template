...
const env = { AUTH_MODE: 'demo', APP_ENV: 'development', BIND_ADDRESS: '127.0.0.1', KONG_HTTP_PORT: '28000', KONG_HTTPS_PORT: '28443', STUDIO_PORT: '55325', DATABASE_HOST_PORT: '25433', GOTENBERG_HOST_PORT: '23100', SUPABASE_PUBLIC_URL: 'http://localhost:28000', API_EXTERNAL_URL: 'http://localhost:28000', CORS_ALLOWED_ORIGIN: 'http://localhost:5173' };
test('doctor validates supported runtime and distinct, matching loopback ports', () => {
  ...
  for (const change of [{ BIND_ADDRESS: '0.0.0.0' }, { AUTH_MODE: 'production' }, { KONG_HTTP_PORT: 'invalid' }, { STUDIO_PORT: '28000' }, { SUPABASE_PUBLIC_URL: 'http://example.com:28000' }, { SUPABASE_PUBLIC_URL: 'http://localhost:18000' }, { CORS_ALLOWED_ORIGIN: '*' }, { CORS_ALLOWED_ORIGIN: 'http://user:pass@localhost:5173' }, { CORS_ALLOWED_ORIGIN: 'http://localhost:5173/path' }]) assert.throws(() => validateDemoEnv({ ...env, ...change }));
});
